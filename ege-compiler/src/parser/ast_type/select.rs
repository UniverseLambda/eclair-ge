...
impl Parsable for Select {
    fn parse(parser: &mut Parser<impl std::io::Read>) -> anyhow::Result<Self> {
        parser
            .required_token()
            .expect_token(TokenTypeId::Keyword, "Select")?;
        ...
        loop {
            ...
            let token = start_token
                ...
                .expect_any_content(&["End", "Case", "Default"])?;

            let values = match token.content.as_str() {
                "End" => break,
                "Case" => {
                    ...
                    loop {
                        ...
                        let next = parser
                            .required_token()
                            .expect_any_type(&[TokenTypeId::EndOfLine, TokenTypeId::Operator])
                            .expect_any_content(&["\n", ","])?;
                        parser.consume_token();
                ...
                "Default" => {
                    if default_case.is_some() {
                        bail!(
                            "{}:{}:{}: default section for this select has already been defined",
                            token.source_path,
                            token.line,
                            token.column
                        );
            ...
            let (statements, stopper) =
                parser.parse_statement_block("End", true, &["Case", "Default"])?;
